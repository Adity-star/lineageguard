...
const IMPACT_RULES: ImpactRule[] = [
  {
    ...
    weight: 70,
    applies: (_, plan) => plan.intent === 'drop_table',
  },

  {
    ...
    weight: 35,
    applies: (_, plan) => plan.intent === 'drop_column',
  },

  {
    ...
    weight: 10,
    applies: (_, plan) => plan.intent === 'rename_column',
  },

  {
    ...
    weight: 5,
    applies: (_, plan) => plan.intent === 'add_column',
  },

  {
    ...
    weight: 20,
    applies: (context) => context.lineage.downstream.length > 5,
  },

  {
    ...
    weight: 20,
    applies: (context) => context.queries.length > 20,
  },

  {
    ...
    weight: 10,
    applies: (context) => context.dataset.owners.length === 0,
  },

  {
    ...
    weight: 10,
    applies: (context) => context.documents.length === 0,
  },
];
...
export class ImpactScorer {
  score(
    ...
    const triggeredRules = IMPACT_RULES.filter((rule) =>
      rule.applies(context, plan, risk),
    );
  ...
  private determineLevel(score: number): ImpactLevel {
    if (score >= 75) {
      return 'CRITICAL';
    }
