...
import { Recommendation, ImpactLevel } from './types.js';
  ...
  generate(
    ...
    if (context.lineage.downstream.length > 0) {
      recommendations.push({
        ...
        description: `Notify owners of ${context.lineage.downstream.length} downstream datasets before deployment.`,
        required: impact !== 'LOW',
      });
    }
    ...
    if (plan.intent === 'drop_column') {
      recommendations.push({
        id: 'backup-column',
        title: 'Create Backup',
        description: 'Back up affected data before removing the column.',
        required: true,
      });
    }
    ...
    if (context.dataset.owners.length === 0) {
      recommendations.push({
        ...
        description: 'Ownership should be assigned before deployment.',
        required: true,
      });
    }
    ...
    if (impact === 'CRITICAL') {
      recommendations.push({
        ...
        description: 'Execute this migration during a maintenance window.',
        required: true,
      });
    }
