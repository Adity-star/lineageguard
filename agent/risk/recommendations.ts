  ...
  generate(
    ...
    if (metrics.downstreamDatasets > 0) {
      recommendations.push({
        ...
        description:
          `Validate ${metrics.downstreamDatasets} downstream dataset(s) before deployment.`,
      });
    }
    ...
    if (!metrics.hasDocumentation) {
      recommendations.push({
        ...
        description:
          "Document the affected dataset before merging the change.",
      });
    }
    ...
    if (risk === "HIGH" || risk === "CRITICAL") {
      recommendations.push({
        ...
        description:
          "Deploy using a phased rollout with rollback capability.",
      });
    }
    ...
    if (metrics.requiresApproval) {
      recommendations.push({
        ...
        description:
          "Obtain approval before executing this change.",
      });
    }
