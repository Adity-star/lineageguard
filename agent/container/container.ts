  ...
  constructor() {
    ...
    const llmEditor: LLMEditor = {
      editSchema: async (request) => {
        // Enhanced mock implementation for testing that preserves input schema
        // In production, this would use LLMSchemaEditor with a real LLM
        
        if (!request.schema || request.schema.trim().length === 0) {
          // Only generate a new schema if input is empty
          return {
...
model users {
  ...
  createdAt DateTime @default(now())
        ...
        // If schema exists, return it as-is for now (mock behavior)
        // In reality, the LLM would apply changes based on the plan
        logger.info({
          event: "llm_editor_mock_preserving_schema",
          inputSchemaLength: request.schema.length,
          planSummary: JSON.stringify(request.plan).substring(0, 200),
        }, "Mock LLM editor preserving input schema (use real LLMSchemaEditor in production)");
        
        return {
          updatedSchema: request.schema,
        };
      },
    };
