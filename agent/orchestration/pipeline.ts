...
export interface PipelineStage {
   ...
   * Execute this stage.
   */
  execute(state: StateStore, perf?: PerformanceTracker): Promise<void>;
}
...
export class Pipeline {
  ...
  async execute(state: StateStore, perf?: PerformanceTracker): Promise<void> {
    ...
    for (let i = 0; i < this.stages.length; i++) {
      ...
      const stageTitle =
        stageTitles[stage.name] ||
        `${stage.name.charAt(0).toUpperCase() + stage.name.slice(1)} Stage`;
      ...
      const hasGeneration = !!stateBefore.generation;
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
State before: context=${hasContext}, plan=${hasPlan}, risk=${hasRisk}, generation=${hasGeneration}, impact=${hasImpact}, approval=${hasApproval}, github=${hasGithub}`,
      );
      ...
      try {
        ...
        const contextKeys = stateAfter.context
          ? Object.keys(stateAfter.context).join(', ')
          : 'none';
        const planKeys = stateAfter.plan
          ? Object.keys(stateAfter.plan).join(', ')
          : 'none';
        const riskKeys = stateAfter.risk
          ? Object.keys(stateAfter.risk).join(', ')
          : 'none';
        const generationOutput = stateAfter.generation
          ? JSON.stringify(stateAfter.generation).substring(0, 150) + '...'
          : 'none';
        const impactOutput = stateAfter.impact
          ? Object.keys(stateAfter.impact).join(', ')
          : 'none';
        const approvalOutput = stateAfter.approval
          ? JSON.stringify(stateAfter.approval).substring(0, 100) + '...'
          : 'none';
        ...
        logger.info(
          {
            ...
            stateAfter: {
              ...
              hasGeneration: !!stateAfter.generation,
...
State after:
  ...
  Plan:     ${planKeys}
  Risk:     ${riskKeys}
  Generation: ${generationOutput.substring(0, 80)}...
  Impact:   ${impactOutput}
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`,
        ...
        throw new Error(
          `Stage "${stage.name}" (${stageIndex}/${totalStages}) failed: ${errorMessage}`,
        );
      }
    }
