...
import { Pipeline, PipelineStage } from './pipeline.js';
...
import { WorkflowEvent, WorkflowListener } from './events.js';
    ...
    logger.info(
      {
        ...
        request: {
          ...
          sql:
            request.sql?.substring(0, 200) +
            (request.sql && request.sql.length > 200 ? '...' : ''),
...
${
  state.value.github
    ? `GitHub PR:     #${state.value.github.number}
...
State at failure: ${Object.keys(state.value).join(', ')}`,
    ...
    return withIdempotency<WorkflowState>(
      ...
      (result) =>
        result.github?.number ? String(result.github.number) : undefined,
    );
  }
}
