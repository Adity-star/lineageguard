...
export class DataHubTagClient {
  ...
  private async executeGraphQL(query: string, variables: Record<string, any> = {}): Promise<any> {
    ...
    if (!this.gmsUrl || !this.token) {
      throw new Error("DataHub GMS URL and token are required for GraphQL operations");
    }
   ...
   */
  async ensureTag(tagUrn: string, name: string, description: string): Promise<boolean> {
    try {
      logger.info({ 
