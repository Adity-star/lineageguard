...
import { Dataset, DatasetSchema, MCPToolResponse } from '../types.js';
...
export class EntityTool {
   ...
   */
  public async getDataset(urn: string): Promise<Dataset> {
    try {
      logger.info(
        {
      ...
      logger.debug(
        {
          ...
          responseLength: Array.isArray(response.data)
  ...
  private mapEntityToDataset(entity: any, providedUrn?: string): Dataset {
    ...
    if (!name && entity.entity) {
      name =
        entity.entity.name || entity.entity.title || entity.entity.displayName;
    }
  ...
  private extractOwners(
    entity: any,
  ): Array<{ urn: string; name: string; type: string }> {
  ...
  private extractTags(entity: any): string[] {
    ...
    if (entity.tags && Array.isArray(entity.tags)) {
      for (const tag of entity.tags) {
        const tagName =
          typeof tag === 'string' ? tag : tag.name || tag.urn || '';
        if (tagName) {
          tags.push(tagName);
        }
      }
  ...
  private extractGlossaryTerms(
    entity: any,
  ): Array<{ urn: string; name: string; description?: string }> {
  ...
  private extractQuality(
    entity: any,
  ): { passedChecks: number; failedChecks: number } | undefined {
    if (entity.quality) {
      return {
    ...
    if (
      entity.passedChecks !== undefined ||
      entity.failedChecks !== undefined
    ) {
      return {
   ...
   */
  private extractDeprecation(
    entity: any,
  ):
    | { deprecated: boolean; note?: string; decommissionDate?: string }
    | undefined {
  ...
  public async getTags(urn: string): Promise<string[]> {
    try {
      logger.info(
        {
