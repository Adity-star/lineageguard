...
import { SchemaField, MCPToolResponse } from '../types.js';
...
export class SchemaTool {
   ...
   */
  async getSchema(urn: string): Promise<MCPToolResponse<SchemaField[]>> {
    ...
    try {
      ...
      const fieldsArray = MCPResponseUnwrapper.unwrapArray(
        unwrapped,
        toolName,
        'fields',
      );
      ...
      const mappedFields =
        SchemaFieldMapper.mapAndValidateMultiple(fieldsArray);
      ...
      throw new Error(`Schema retrieval failed for ${urn}: ${errorMessage}`);
    }
  }
   ...
   */
  async hasField(urn: string, fieldPath: string): Promise<boolean> {
