...
export class LineageTool {
  ...
  async getUpstream(
    ...
    return this.executeLineageTool("get_lineage", { urn, direction: "upstream" });
  }
  ...
  async getDownstream(
    ...
    return this.executeLineageTool("get_lineage", { urn, direction: "downstream" });
  }
  ...
  private async executeLineageTool(
    ...
    try {
      ...
      const unwrapped = MCPResponseUnwrapper.unwrapObject(rawResponse, toolName);
      ...
      // Log diagnostics
      LineageMapper.logMappingDiagnostics(toolName, args, unwrapped, mapped, durationMs);
