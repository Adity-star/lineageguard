...
import { DatasetQuerySchema, DatasetQuery, MCPToolResponse } from '../types.js';
...
export class QueryTool {
  ...
  async getDatasetQueries(
    ...
    try {
      ...
      const unwrapped = MCPResponseUnwrapper.unwrapArray(
        rawResponse,
        toolName,
        'queries',
      );
      ...
      const mappedQueries = unwrapped
        .map((rawQuery: any, index: number) => {
          try {
      ...
      throw new Error(`Query retrieval failed for ${urn}: ${errorMessage}`);
    }
  }
  ...
  async searchQueries(
    ...
    try {
      ...
      // Execute raw to get unwrapped response
      const rawResponse = await this.client.executeToolRaw(toolName, {
        keyword,
      });
  ...
  private mapQuery(rawQuery: any): DatasetQuery {
    if (!rawQuery || typeof rawQuery !== 'object') {
      throw new Error(
        `Invalid query object: expected object, got ${typeof rawQuery}`,
      );
    }

    // Extract ID
    const id =
      rawQuery.id || rawQuery.queryId || rawQuery.query_id || rawQuery.urn;
    if (!id) {
      throw new Error(
