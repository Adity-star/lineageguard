...
export class GitHubEngine {
  ...
  async execute(
    ...
    const headBranch = request.branchName || this.branchBuilder.build(request.plan);
    ...
    try {
      ...
      if (!verifyBranch.exists) {
        const error = new Error(`GitHub feature branch was not created: ${request.owner}/${request.repository}:${headBranch}`);
        logger.error({
   ...
   */
  private logGitHubError(error: any, request: GitHubEngineRequest, operation: string): void {
