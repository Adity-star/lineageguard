...
export class PullRequestBuilder {
  ...
  build(
    ...
    return {
      branch: this.branchBuilder.build(plan),

      title: this.commitBuilder.build(plan),

      body: this.template.build(generation, impact),

      labels: this.labels.resolve(impact),

      reviewers: this.reviewers.resolve(context).map((username) => ({
        username,
      })),
    };
  }
