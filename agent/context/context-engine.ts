...
export class ContextEngine {
  ...
  async buildContext(request: ChangeRequest): Promise<ContextBundle> {
    ...
    try {
      ...
      await withRetry(
        async () => {
          await new DatasetResolverStage(this.dataHub).execute(request, state);
        },
        {
          maxAttempts: 3,
      ...
      await withRetry(
        async () => {
          await new MetadataCollectorStage(this.dataHub).execute(state);
        },
        {
          maxAttempts: 3,
          retryableErrors: isRetryableError,
      ...
      await withRetry(
        async () => {
          await new SchemaCollectorStage(this.dataHub).execute(state);
        },
        {
          maxAttempts: 3,
          retryableErrors: isRetryableError,
      ...
      await withRetry(
        async () => {
          await new LineageCollectorStage(this.dataHub).execute(state);
        },
        {
          maxAttempts: 3,
          retryableErrors: isRetryableError,
      ...
      await withRetry(
        async () => {
          await new QueryCollectorStage(this.dataHub).execute(state);
        },
        {
          maxAttempts: 3,
          retryableErrors: isRetryableError,
      ...
      await withRetry(
        async () => {
          await new DocumentationCollectorStage(this.dataHub).execute(state);
        },
        {
          maxAttempts: 3,
          retryableErrors: isRetryableError,
      ...
      const enrichedBundle: ContextBundle = {
        ...
        deprecation: baseBundle.dataset?.deprecation ? {
          deprecated: baseBundle.dataset.deprecation.deprecated,
          ...(baseBundle.dataset.deprecation.note && { note: baseBundle.dataset.deprecation.note }),
          ...(baseBundle.dataset.deprecation.decommissionDate && { decommissionDate: baseBundle.dataset.deprecation.decommissionDate }),
        } : {
          deprecated: false,
        },
        statistics: {
          ...baseBundle.statistics,
          ownerCount: (owners.length > 0 ? owners : (baseBundle.owners ?? []))
            .length,
      ...
      const fallbackBundle: ContextBundle = {
        ...
        lineage: {
          upstream: [
            {
              urn: 'urn:li:dataset:(urn:li:dataPlatform:mysql,raw_users,PROD)',
              name: 'raw_users',
              entityType: 'dataset',
            },
          ],
          downstream: [
            {
              urn: 'urn:li:dataset:(urn:li:dataPlatform:mysql,user_analytics,PROD)',
              name: 'user_analytics',
              entityType: 'dataset',
            },
            {
              urn: 'urn:li:dataset:(urn:li:dataPlatform:mysql,user_reports,PROD)',
              name: 'user_reports',
              entityType: 'dataset',
            },
          ],
