...
export class ContextNormalizerStage {
  execute(state: ContextState): ContextBundle {
    if (!state.dataset || !state.schema || !state.lineage) {
    ...
    return {
      ...
      statistics: {
        totalFields: state.schema.length,

        upstreamCount: state.lineage.upstream.length,

        downstreamCount: state.lineage.downstream.length,
      ...
      deprecation: state.dataset?.deprecation ? {
        deprecated: state.dataset.deprecation.deprecated,
        ...(state.dataset.deprecation.note && { note: state.dataset.deprecation.note }),
        ...(state.dataset.deprecation.decommissionDate && { decommissionDate: state.dataset.deprecation.decommissionDate }),
      } : {
        deprecated: false,
      },
