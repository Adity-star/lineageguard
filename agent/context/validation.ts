...
export function validateContextBundle(bundle: ContextBundle): void {
  ...
  if (bundle.statistics && bundle.schema && bundle.lineage) {
    if (
      bundle.statistics.totalFields !== undefined &&
      bundle.statistics.totalFields !== bundle.schema.length
    ) {
      errors.push(
        `Statistics totalFields (${bundle.statistics.totalFields}) does not match schema length (${bundle.schema.length})`,
      );
    }
    if (
      bundle.statistics.upstreamCount !== undefined &&
      bundle.statistics.upstreamCount !== bundle.lineage.upstream.length
    ) {
      errors.push(
        `Statistics upstreamCount (${bundle.statistics.upstreamCount}) does not match lineage upstream length (${bundle.lineage.upstream.length})`,
      );
    }
    if (
      bundle.statistics.downstreamCount !== undefined &&
      bundle.statistics.downstreamCount !== bundle.lineage.downstream.length
    ) {
      errors.push(
        `Statistics downstreamCount (${bundle.statistics.downstreamCount}) does not match lineage downstream length (${bundle.lineage.downstream.length})`,
      );
    }
  }
